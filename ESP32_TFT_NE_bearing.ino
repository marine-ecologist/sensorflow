...
// Create sensor objectss
Adafruit_LIS3MDL lis3mdl; // magnetometer
Adafruit_LSM6DS3TRC lsm6ds3trc; // acceleraton / gyro
...
Serial.print("Accelerometer range set to: ");
  ...
  // Print static label for bearing
  //tft.setCursor(20, 10);
...
void loop() {
  ...
  // Calculate heading in degrees (compass bearing)
  float heading = atan2(lis3mdl.y, lis3mdl.x) * 180 / PI;
  if (heading < 0) {
    heading += 360;
  }
  ...
  // Project acceleration into North and East components
  float north = accel.acceleration.x * cos(pitch) - accel.acceleration.y * sin(roll);
  float east = accel.acceleration.y * cos(roll) + accel.acceleration.x * sin(pitch) * sin(roll);
  ...
  tft.setCursor(10, 10);
  tft.fillRect(0, 10, tft.width(), 18, ST77XX_WHITE); // Draw white rectangle
  tft.setTextColor(ST77XX_BLACK);
  tft.print(" MC-01 Tiltmeter");

// Update TFT display Northing and Easting
  tft.setCursor(10, 40);
  tft.setTextColor(ST77XX_BLACK, ST77XX_BLACK);
  tft.print("        "); 
  tft.setCursor(10, 40);
  tft.setTextColor(ST77XX_CYAN, ST77XX_BLACK); tft.print("N: "); tft.setTextColor(ST77XX_WHITE, ST77XX_BLACK); tft.print(north, 2); //tft.print("char(248)");
  tft.setTextColor(ST77XX_BLUE, ST77XX_BLACK); tft.print(" / E: "); tft.setTextColor(ST77XX_WHITE, ST77XX_BLACK); tft.print(east, 2); //tft.print("char(248)");


  // Update TFT display bearing
  tft.setCursor(10, 60);
  tft.setTextColor(ST77XX_BLACK, ST77XX_BLACK);
  tft.print("        "); 
  tft.setCursor(10, 60);
  tft.setTextColor(ST77XX_GREEN, ST77XX_BLACK);
  tft.print("Bearing: "); tft.setTextColor(ST77XX_WHITE, ST77XX_BLACK); tft.print(heading, 2); //tft.print("char(248)");
  ...
  tft.print(temp.temperature, 2); //tft.println(" deg C");


  // Update TFT display time and battery
